...
#if ENABLE_DSI_INTERRUPT
    long int time;
//Begin fhq[2015-3-31, modified for quick suspend resume]
    //static const long WAIT_TIMEOUT = 2 * HZ;    // 2 sec
    static const long WAIT_TIMEOUT = HZ/5;//2 * HZ;    // 2 sec
//End fhq
#endif
    ...
    else
    {
        while (DSI_REG->DSI_INTSTA.BUSY || DSI_REG->DSI_INTSTA.CMD_DONE)
        {
            long ret = wait_event_interruptible_timeout(_dsi_wait_queue,
                                                        ...
                                                        WAIT_TIMEOUT);
            if (0 == ret) {
		printk(KERN_ERR "Hollen %s DSI_REG->DSI_INTSTA.BUSY || DSI_REG->DSI_INTSTA.CMD_DONE\n", __func__);
    ...
    while(_IsEngineBusy()) {
        ...
        if (--timeOut < 0) {
		printk(KERN_ERR "Hollen %s DSI_INTERRUPT not enable\n", __func__);
...
{
    printk("BLS: 0x%x\n", INREG32(0xF400A000));
    return (0x1 == (INREG32(0xF400A000)&0x1));//if 1, BLS enable
}
