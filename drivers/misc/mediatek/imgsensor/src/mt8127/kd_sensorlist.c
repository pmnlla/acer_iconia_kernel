...
/*******************************************************************************
  ...
  * Used to dump some critical sensor register
  ********************************************************************************/
static ssize_t CAMERA_HW_DumpReg_To_Proc(struct file *file, char __user *pPage, size_t count,
					 loff_t *ppos)
{
    return count;
}
//add clj 20150422
static ssize_t CAMERA_HW_DumpReg_To_Proc3(struct file *file, char __user *pPage, size_t count,
					 loff_t *ppos)
{
    return count;
}
...
/*******************************************************************************
  ...
  ********************************************************************************/
static ssize_t CAMERA_HW_Reg_Debug(struct file *pFile, const char __user *buffer, size_t count,
				   loff_t *ppos)
{
    char regBuf[64] = {'\0'};
...
//add clj 20150422
static ssize_t CAMERA_HW_Reg_Debug3(struct file *pFile, const char __user *buffer, size_t count,
				   loff_t *ppos)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
...
static const struct file_operations g_stCAMERA_camsensor_fops = {
	.owner = THIS_MODULE,
	.read = CAMERA_HW_DumpReg_To_Proc,
	.write = CAMERA_HW_Reg_Debug
};
//add clj 20150422
static const struct file_operations g_stCAMERA_camsensor_fops_s = {
	.owner = THIS_MODULE,
...
//add clj 20150422
    psEntry = proc_create("driver/camsensor3", 0, NULL, &g_stCAMERA_camsensor_fops_s);
	if (!prEntry) {
		PK_ERR("add /proc/driver/camsensor3 entry fail\n");
	}
